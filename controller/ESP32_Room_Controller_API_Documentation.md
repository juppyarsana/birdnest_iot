...
- **HTTP**: Still available for WiFi setup UI and optional debugging API
- **Device Name**: Birdnest Room Controller
- **Manufacturer**: Orca Technology

## MQTT Configuration

- **Broker URL**: `mqtt://<broker_ip>:1883`
- **Client ID**: `birdnest-<device_id>`
- **Base Topic**: `birdnest/controllers/<client_id>`
- **Subscriptions**:
  - `<base_topic>/cmd/#`
  - `birdnest/cmd/all/#`
- **Publications**:
  - `<base_topic>/availability` (retained): `online` | `offline`
  - `<base_topic>/info` (retained): device metadata JSON
  - `<base_topic>/status` (retained): full state snapshot JSON
  - `<base_topic>/ack` (not retained): command acknowledgement JSON
  - `<base_topic>/ir/list` (not retained): IR list response JSON

### MQTT Status Payload

Published to `<base_topic>/status` (retained).

```json
{
  "device": {
    "id": "birdnest-ABCD1234EF567890"
  },
  "wifi": {
    "connected": true,
    "ssid": "YourWiFiName",
...
Commands are sent by publishing a JSON payload to:

- `<base_topic>/cmd/<command>`
- or `birdnest/cmd/all/<command>` (broadcast)

After processing a command, the controller publishes:

- `<base_topic>/ack` with an acknowledgement JSON
- `<base_topic>/status` with the updated snapshot (retained)

#### 1. Light Control

- **Topic**: `<base_topic>/cmd/light`
- **Payload**:
...
`slot` is optional. When provided, existing command data in that slot is removed before learning starts.

#### 6. IR Stop

- **Topic**: `<base_topic>/cmd/ir/stop`
- **Payload**:
...
- **Response Topic**: `<base_topic>/ir/list`
- **Response Payload**:

```json
{
  "commands": [
    {"slot": 0, "name": "CMD_0", "type": "raw"}
  ],
  "count": 1,
  "max": 10
}
...
## HTTP Configuration (Legacy / Debug)
